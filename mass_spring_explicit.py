...
f = ti.Vector.field(2, dtype=ti.f32, shape=max_num_particles)#受力大小，在下方会使用f表示f/m的加速度，由于m=1默认于是就直接用f表示加速度
...
rest_length = ti.field(dtype=ti.f32,
                       shape=(max_num_particles, max_num_particles)) #弹簧静态长度
...
def new_particle(pos_x: ti.f32, pos_y: ti.f32, fixed_: ti.i32):
    ...
    for i in range(new_particle_id):
        ...
        if dist < connection_radius: #两粒子小于某一阈值就加弹簧
            # Connect the new particle with particle i
            rest_length[i, new_particle_id] = 0.1 #这两行表示对称矩阵
...
def main():
    ...
    while True:
        ...
        # Draw the springs
        for i in range(n):#n = num_particles[None]
